...
public class DistanceMapRequest extends TravelTimeRequest<DistanceMapResponse> {
    @Valid
    @Singular
    ...
    public Either<TravelTimeError, Request> createRequest(HttpUrl baseUri, TravelTimeCredentials credentials) {
        val uri = baseUri.newBuilder().addPathSegments("distance-map").build();
        return JsonUtils.toJson(this)
                .map(json -> createPostRequest(credentials, uri, json, AcceptType.APPLICATION_JSON));
    }

    @Override
    public Class<DistanceMapResponse> responseType() {
        return DistanceMapResponse.class;
    }
}
