...
public class TimeFilterFastProtoRequest extends ProtoRequest<TimeFilterFastProtoResponse> {
  ...
  @NonNull @Singular List<Coordinates> destinationCoordinates;
   ...
   * @param destinationCoordinates The coordinates of locations we run the search to. If the class
   *     implementing this list does not implement the {@code RandomAccess} interface it will be
   *     internally converted into an {@code ArrayList}.
  ...
  private byte[] createByteArray() {
    ...
    RequestsCommon.Coords source =
        RequestsCommon.Coords.newBuilder()
    ...
    if (requestType == RequestType.ONE_TO_MANY) {
      TimeFilterFastRequest.OneToMany.Builder oneToManyBuilder =
          TimeFilterFastRequest.OneToMany.newBuilder()
      ...
      for (Coordinates dest : this.destinationCoordinates) {
        oneToManyBuilder.addLocationDeltas(
            (int) Math.round((dest.getLat() - origin.getLat()) * mult));
        oneToManyBuilder.addLocationDeltas(
            (int) Math.round((dest.getLng() - origin.getLng()) * mult));
      }
      ...
      TimeFilterFastRequest.ManyToOne.Builder manyToOneBuilder =
          TimeFilterFastRequest.ManyToOne.newBuilder()
      ...
      double mult = Math.pow(10, 5);
      for (Coordinates dest : this.destinationCoordinates) {
        manyToOneBuilder.addLocationDeltas(
            (int) Math.round((dest.getLat() - origin.getLat()) * mult));
        manyToOneBuilder.addLocationDeltas(
            (int) Math.round((dest.getLng() - origin.getLng()) * mult));
      }
  ...
  public List<ProtoRequest<TimeFilterFastProtoResponse>> split(int batchSizeHint) {
    ...
    if (this.destinationCoordinates.size() <= batchSizeHint * (loadFactor + 1)) {
      ...
      ArrayList<ProtoRequest<TimeFilterFastProtoResponse>> batchedDestinations =
          new ArrayList<>(batchCount);

      for (int offset = 0; offset < this.destinationCoordinates.size(); offset += batchSize) {
        List<Coordinates> batch =
            this.destinationCoordinates.subList(
