...
public interface Transportation {
  ...
  class Modes {
    public static final PublicTransport PUBLIC_TRANSPORT = PublicTransport.builder().build();
    ...
    public static final Transportation WALKING =
        new TransportationWithoutDetails(TransportationType.Types.WALKING);
    public static final Transportation CYCLING =
        new TransportationWithoutDetails(TransportationType.Types.CYCLING);
    public static final Transportation DRIVING =
        new TransportationWithoutDetails(TransportationType.Types.DRIVING);
  }
  ...
  class PublicTransport implements Transportation {
    ...
    /**
     * limits the possible duration of walking paths must be > 0 and <= 1800 walkingTimeToStation
     * limit is of low precedence and will not override the global travel time limit If not set,
     * server side decides what `walkingTimeToStation` value should be which is a reasonable value
     * within the [1, 1800] (inclusive)
     */
    @Builder.Default @With private Integer walkingTimeToStation = null;
      ...
      return RequestsCommon.Transportation.newBuilder()
          .setPublicTransport(
              RequestsCommon.PublicTransportDetails.newBuilder()
                  .setWalkingTimeToStation(
                      this.walkingTimeToStation == null ? 0 : this.walkingTimeToStation))
  ...
  class DrivingAndPublicTransport implements Transportation {
    ...
    /**
     * limits the possible duration of driving paths must be > 0 and <= 1800 drivingTimeToStation
     * limit is of low precedence and will not override the global travel time limit If not set,
     * server side decides what `drivingTimeToStation` value should be which is a reasonable value
     * within the [1, 1800] (inclusive)
     */
    @Builder.Default @With private Integer drivingTimeToStation = null;

    /**
     * constant penalty to apply to simulate the difficulty of finding a parking spot. If
     * parkingTime >= 0: apply the parking penalty when searching for possible paths. parkingTime
     * penalty cannot be greater than the global travel time limit If not set, server side decides
     * what `parkingTime` value should be
     */
    @Builder.Default @With private Integer parkingTime = null;
          ...
          .setDrivingAndPublicTransport(
              RequestsCommon.DrivingAndPublicTransportDetails.newBuilder()
                  ...
                  .setDrivingTimeToStation(
                      this.drivingTimeToStation == null ? 0 : this.drivingTimeToStation)
  ...
  interface TransportationType {
    ...
    enum Types implements TransportationType {
      ...
      DRIVING_AND_PUBLIC_TRANSPORT(
          "pt", RequestsCommon.TransportationType.DRIVING_AND_PUBLIC_TRANSPORT_VALUE),
