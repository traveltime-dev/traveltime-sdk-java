...
public class TimeFilterFastProtoTest {
  ...
  public void shouldSendTimeFilterProtoRequest() {
    ...
    List<Coordinates> destinations =
        Collections.singletonList(new Coordinates(51.348605, -0.314783));
  ...
  public void shouldReturnDistanceWhenFlagIsSpecified() {
    ...
    TimeFilterFastProtoRequest request =
        new TimeFilterFastProtoRequest(
            origin,
            destinations,
  ...
  public void shouldSplitProtoRequestsTest() {
    ...
    Assert.assertTrue(
        requests.stream().allMatch(req -> req.getDestinationCoordinates().size() <= 3));
  }

  @Test
  public void shouldSplitProtoRequestsEvenlyTest() {
    TimeFilterFastProtoRequest request =
        oneToMany(new Coordinates(51.425709, -0.122061), Common.generateCoordinates(1001));

    List<ProtoRequest<TimeFilterFastProtoResponse>> requests = request.split(100);
    Assert.assertTrue(
        requests.stream().allMatch(req -> req.getDestinationCoordinates().size() != 1));
  }

  @Test
  public void shouldSendAsyncTimeFilterProtoRequest()
      throws ExecutionException, InterruptedException {
    Coordinates origin = new Coordinates(51.425709, -0.122061);
    List<Coordinates> destinations =
        Collections.singletonList(new Coordinates(51.348605, -0.314783));
    TimeFilterFastProtoRequest request = oneToMany(origin, destinations);
    CompletableFuture<Either<TravelTimeError, TimeFilterFastProtoResponse>> response =
        sdk.sendProtoAsync(request);
  ...
  public void shouldSendBatchTimeFilterProtoRequest() {
    ...
    TimeFilterFastProtoRequest request = oneToMany(origin, destinations);
    Either<TravelTimeError, TimeFilterFastProtoResponse> response =
        sdk.sendProtoBatched(request, 3);
    Common.assertResponseIsRight(response);
  ...
  public void shouldSendMultipleAsyncTimeFilterProtoRequests() {
    ...
    List<CompletableFuture<Either<TravelTimeError, TimeFilterFastProtoResponse>>> futures =
        Arrays.asList(
  ...
  public void shouldSendRequestWithCustomTransportationDetails() {
    ...
    List<Coordinates> destinations =
        Collections.singletonList(new Coordinates(51.348605, -0.314783));
    Transportation.DrivingAndPublicTransport transportation =
        Transportation.DrivingAndPublicTransport.builder()
  ...
  public TimeFilterFastProtoRequest manyToOne(Coordinates origin, List<Coordinates> destinations) {
  ...
  public TimeFilterFastProtoRequest oneToMany(Coordinates origin, List<Coordinates> destinations) {
    return new TimeFilterFastProtoRequest(
        origin,
        destinations,
        Transportation.Modes.PUBLIC_TRANSPORT,
