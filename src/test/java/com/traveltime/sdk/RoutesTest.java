...
public class RoutesTest {
  ...
  public void shouldSendRoutesRequests() {
    ...
    List<Transportation> transportationModes =
        Arrays.<Transportation>asList(
            PublicTransport.builder().build(),
            Driving.builder().trafficModel(DrivingTrafficModel.OPTIMISTIC).build());

    transportationModes.forEach(
        transportation -> {
          RoutesRequest request =
              new RoutesRequest(
                  locations,
                  createDepartureSearch(
                      "location1", Arrays.asList("location2", "location3"), transportation),
                  createArrivalSearch(
                      Arrays.asList("location2", "location3"), "location1", transportation));
